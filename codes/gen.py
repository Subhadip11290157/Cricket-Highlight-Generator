...
energy = np.array([sum(abs(vid[i:i+window_length]**2))
                  for i in range(0, len(vid), window_length)])
plt.hist(energy)
plt.show()
...
for i in range(len(df)):
    ...
    # Enter your sports video clip name here.
    ffmpeg_extract_subclip("video.mp4", start_lim, end_lim,
                           targetname=sub_folder+"/"+filename)
...
# Enter the desired output highlights filename.
final_clip.write_videofile("Yess.mp4")
shutil.rmtree(sub_folder)  # Delete the temporary file.


"""
Follow this to delete clips without error:
https://github.com/Zulko/moviepy/issues/810
